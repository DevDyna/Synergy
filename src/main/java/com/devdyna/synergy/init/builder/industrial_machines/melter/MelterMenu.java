...
public class MelterMenu extends BaseMachineMenu {
    ...
    public final MelterBE blockEntity;
    ...
    public MelterMenu(int i, Inventory inv, BlockEntity be, ContainerData data) {
        super(zMachines.MELTER.menu().get(), i, be);
        this.blockEntity = ((MelterBE) be);
        this.level = inv.player.level();
        this.data = data;
        addPlayerSlots(inv);
        addMachineInputSlot(blockEntity.getStorage(), MelterBE.INPUT_SLOT, 47, 33);
        // addMachineOutputSlot(blockEntity.getStorage(), MelterBE.OUTPUT_SLOT, 119, 34);
    ...
    @Override
    public MelterBE getBlockEntity() {
        return blockEntity;
    }
