...
public class GlowstoneCooler extends CoolerBlockBase {
    ...
    public boolean activeWhen(BlockState state, Level level, BlockPos pos) {
        ...
        for (int i = 0; i < Direction.values().length; i++) {
            ...
            if (block instanceof ModeratorBase mod)
                if(mod.isActive(level, pos.relative(Direction.values()[i])))
                moderators++;
        }
        return moderators >= 2;
    }
    ...
    public Component conditions() {
        return Component.translatable(Main.ID + "." + zStatic.ReactorStuff.cooler +"." + zStatic.ReactorStuff.CoolerTypes.GLOWSTONE);
    }
