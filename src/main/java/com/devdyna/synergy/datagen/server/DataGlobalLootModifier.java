...
public class DataGlobalLootModifier extends GlobalLootModifierProvider {
        ...
        protected void asignItemToLootTable(DeferredHolder<Item, Item> item) {
                asignItemToLootTable(item, DataAnyLoot.MOB_DROPS.indexOf(item));
        }

        protected void asignItemToLootTable(DeferredHolder<Item, Item> item, int index) {
                asignItemToLootTable(item, index, index);
        }

        protected void asignItemToLootTable(DeferredHolder<Item, Item> item, int chestIndex, int mobIndex) {
                DataGenUtil.modifyLootTables(this,
                                item.getRegisteredName().replace(ID + ":", DataAnyLoot.PREFIX_DROPS),
                                CHEST_TYPES.get(chestIndex).toArray(ResourceKey[]::new),
                                MOB_TYPES.get(mobIndex).toArray(EntityType<?>[]::new));
        }
